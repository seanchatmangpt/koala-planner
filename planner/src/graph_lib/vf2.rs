...
pub fn vf2_isomorphism(
    ...
    while !fringe.is_empty() {
        ...
        let in_1: HashSet<u32> = state
            .iter()
            .map(|(n1, _)| g1.get_incoming_edges(*n1))
            .flatten()
            .collect();
        let in_2: HashSet<u32> = state
            .iter()
            .map(|(_, n2)| g2.get_incoming_edges(*n2))
            .flatten()
            .collect();
        // immediate predecessors of state
        let out_1: HashSet<u32> = state
            .iter()
            .map(|(n1, _)| g1.get_outgoing_edges(*n1))
            .flatten()
            .collect();
        let out_2: HashSet<u32> = state
            .iter()
            .map(|(_, n2)| g2.get_outgoing_edges(*n2))
            .flatten()
            .collect();
        ...
        // // if both are empty
            ...
            let p1_list: Vec<u32> = g1
                .nodes
                .difference(&BTreeSet::from_iter(state.iter().map(|(x, _)| *x)))
                .cloned()
                .collect();
            let p2_list: Vec<u32> = g2
                .nodes
                .difference(&BTreeSet::from_iter(state.iter().map(|(_, x)| *x)))
                .cloned()
                .collect();
        ...
        for (n, m) in p.iter() {
            ...
            let pred_n_list: BTreeSet<u32> = g1
                .get_incoming_edges(*n)
                .iter()
                .map(|x| *l1.get(x).unwrap())
                .collect();
            let pred_m_list: BTreeSet<u32> = g2
                .get_incoming_edges(*m)
                .iter()
                .map(|x| *l2.get(x).unwrap())
                .collect();
            if pred_n_list != pred_m_list {
                continue;
            }
            // assert same successors
            let succ_n_list: BTreeSet<u32> = g1
                .get_outgoing_edges(*n)
                .iter()
                .map(|x| *l1.get(x).unwrap())
                .collect();
            let succ_m_list: BTreeSet<u32> = g2
...
mod tests {
    ...
    pub fn vf2_correctness_test() {
        ...
        let _result = vf2_isomorphism(&g1, &g2, &l1, &l2);
